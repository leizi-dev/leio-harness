"""Locate the bundled Leio Harness SDK runtime shipped with this package.
...
- **node (dev-only)**: the full deploy closure under ``runtime/node/``
  ...
  runtime/node/node_modules/@leio-ai/leio-sdk-jsonrpc-demo/lib/packaged-bin.js`` on a
...
def resolve_bundled_launch_args(mode: str | None = None) -> tuple[str, ...]:
    ...
    raise ValueError(
        f"unsupported Leio Harness runtime mode {selected!r}: expected 'exe' or 'node' "
