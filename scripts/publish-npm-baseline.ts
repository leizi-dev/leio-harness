...
const RELEASE_ENTRY_PACKAGE = '@leio-ai/leio'
...
class InstalledBundleSmoke {
  ...
  run(): void {
    ...
    try {
      ...
      const bin = resolve(consumerRoot, 'node_modules/@leio-ai/leio/lib/bin.js')
...
function parsePackedPackage(value: unknown, index: number): PackedPackage {
  ...
  if (origin === 'harness' && (!name.startsWith('@deepseek-ai/') || name === '@leio-ai/leio-root')) {
