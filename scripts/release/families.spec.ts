...
describe('release families', () => {
  it('names one tag for the whole dsh family and one per vendored package', () => {
    ...
    const cli = member('apps/cli', '@leio-ai/leio')
  ...
  it('rejects a family whose members disagree on the shared version', () => {
    const dsh = releaseFamily('dsh')
    const members = [member('apps/cli', '@leio-ai/leio'), { ...member('apps/web', '@leio-ai/leio-web-frontend'), version: '0.0.2' }]
  ...
  it('publishes a dependency before its consumer, and orders ties by name', () => {
    const dsh = releaseFamily('dsh')
    const members = [
      member('packages/a/consumer', '@leio-ai/leio-consumer', { dependencies: { '@leio-ai/leio-library': 'workspace:^' } }),
      member('packages/a/library', '@leio-ai/leio-library'),
      member('packages/a/zebra', '@leio-ai/leio-zebra'),
    ]

    expect(dsh.publishOrder(members).map(entry => entry.name)).toEqual([
      '@leio-ai/leio-library',
      '@leio-ai/leio-consumer',
      '@leio-ai/leio-zebra',
    ])
  })
  ...
  it('reports a runtime dependency cycle instead of emitting an arbitrary order', () => {
    const dsh = releaseFamily('dsh')
    const members = [
      member('packages/a/left', '@leio-ai/leio-left', { dependencies: { '@leio-ai/leio-right': 'workspace:^' } }),
      member('packages/a/right', '@leio-ai/leio-right', { dependencies: { '@leio-ai/leio-left': 'workspace:^' } }),
    ]
  ...
  it('applies the harness payload policy to dsh and keeps upstream payloads for vendored packages', () => {
    const dsh = releaseFamily('dsh')
    const vendor = releaseFamily('vendor')
    const harness = member('packages/a/library', '@leio-ai/leio-library')
  ...
  it('drives the installed entry only for the family that publishes one', () => {
    expect(releaseFamily('dsh').installedEntry).toEqual({ packageName: '@leio-ai/leio', binPath: 'lib/bin.js' })
...
describe('payload change judgement', () => {
  ...
  it('ignores paths no tarball carries', () => {
    ...
    expect(reachesPayload(member('packages/a/library', '@leio-ai/leio-library', { files: ['lib/index.js'] }),
