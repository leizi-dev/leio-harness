...
function clientConfigs(id = '@leio-ai/leio-client-test') {
...
describe('client bundle build faces', () => {
  it('watches source in development and consumes emitted JavaScript in the Client build', () => {
    const bundle = clientBundle('@leio-ai/leio-client-test', ['lib/types/index.js'])
...
describe('client bundle purity gate', () => {
  ...
  it('leaves platform table entries and non-scoped specifiers alone', () => {
    expect(resolveId('@leio-ai/leio-client-ui-slots')).toBeNull()
    expect(resolveId('@leio-ai/leio-client-web-react')).toBeNull()
    expect(resolveId('@leio-ai/leio-client-ui-primitives')).toBeNull()
    expect(resolveId('react')).toBeNull()
    expect(resolveId('zod')).toBeNull()
  })

  it('rejects retired table entries (web-react/store left the 8-entry seed)', () => {
    expect(() => resolveId('@leio-ai/leio-client-web-react/store')).toThrow(/purity/)
  })

  it('lets inline-safe wire layers inline', () => {
    expect(resolveId('@leio-ai/leio-host-apiproxy/api')).toBeNull()
    expect(resolveId('@leio-ai/leio-session/surface')).toBeNull()
    expect(resolveId('@leio-ai/leio-brand')).toBeNull()
  })

  it('lets exact generated Remote contributions inline without admitting their package implementation', () => {
    expect(resolveId('@leio-ai/leio-goal/remote')).toBeNull()
    expect(() => resolveId('@leio-ai/leio-goal')).toThrow(/purity/)
    expect(() => resolveId('@leio-ai/leio-goal/client')).toThrow(/purity/)
    expect(() => resolveId('@leio-ai/leio-goal/remote/nested')).toThrow(/purity/)
  })

  it('throws on any other @deepseek-ai leak', () => {
    expect(() => resolveId('@leio-ai/leio-agent')).toThrow(/purity/)
    expect(() => resolveId('@leio-ai/leio-client-web')).toThrow(/purity/)
  })

  it('throws on cross-plugin value imports — bare plugin names and /client subpaths alike (the rewrite arm is gone)', () => {
    expect(() => resolveId('@leio-ai/leio-client-connection')).toThrow(/purity/)
    expect(() => resolveId('@leio-ai/leio-client-runtime')).toThrow(/purity/)
    expect(() => resolveId('@leio-ai/leio-client-ui-layout/client')).toThrow(/purity/)
  })

  it('carries exactly one documented temporary exemption: runtime/client (store engine pending rehoming)', () => {
    expect(resolveId('@leio-ai/leio-client-runtime/client')).toBeNull()
    const clientChannels = CLIENT_EXTERNALS.filter(
      entry => entry.startsWith('@deepseek-ai/') && entry.endsWith('/client'))
    expect(clientChannels).toEqual(['@leio-ai/leio-client-runtime/client'])
  })
})
...
describe('client bundle debug artifacts', () => {
  ...
  it('maps first-party sources to their repository package paths', () => {
    const configs = clientConfigs('@leio-ai/leio-client-ui-goal')
    ...
    const resolved = new URL(source, 'https://dsh.test/plugins/@leio-ai/leio-client-ui-goal/client.js.map')
  ...
  it('maps dual-face host sources to the host package group', () => {
    const configs = clientConfigs('@leio-ai/leio-host-directory-picker-native')
    const outputOptions = configs[0]?.outputOptions
    if (typeof outputOptions !== 'object' || outputOptions === null) throw new Error('client output options missing')
    const transform = outputOptions.sourcemapPathTransform
    if (transform === undefined) throw new Error('client sourcemap path transform missing')
  ...
  it('maps inlined workspace sources to packages and leaves dependencies outside it unchanged', () => {
    const configs = clientConfigs('@leio-ai/leio-client-connection')
    const outputOptions = configs[0]?.outputOptions
    if (typeof outputOptions !== 'object' || outputOptions === null) throw new Error('client output options missing')
    const transform = outputOptions.sourcemapPathTransform
    if (transform === undefined) throw new Error('client sourcemap path transform missing')
    ...
    const resolved = new URL(workspaceSource, 'https://dsh.test/plugins/@leio-ai/leio-client-connection/client.js.map')
