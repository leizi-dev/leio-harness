...
const SESSION_PACKAGE = '@leio-ai/leio-session'

/** The type-only module that plugin declaration merges augment. */
const SESSION_TYPES_MODULE = '@leio-ai/leio-session/types'
 ...
 * top-level declaration (in `@leio-ai/leio-session`) and any declaration
 * merge inside a `declare module '@leio-ai/leio-session/types'` block. Both forms
...
export function renderKnownEventTypes(events: AnnotatedLogEventEntry[]): string {
  ...
  return [
    ...
    ' * @module @leio-ai/leio-session/known-event-types',
    ' */',
    '',
    '/**',
