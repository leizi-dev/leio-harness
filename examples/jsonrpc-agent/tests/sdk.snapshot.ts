 ...
 * REAL `@leio-ai/leio-sdk-client`, drives one turn over stdio JSON-RPC,
...
} from '@leio-ai/leio-acp-snapshot'
import { resolveExampleLaunch } from '@leio-ai/leio-loader-smoke'
import { DeepSeekHarness, type HarnessNotification, type RunResult } from '@leio-ai/leio-sdk-client'
...
function assembledRuntimeContexts(log: PersistedLog): string[] {
  return log.content.trimEnd().split('\n').flatMap((line) => {
    ...
    if (event.type !== 'user/message'
      ...
      || event.data.source.plugin !== '@leio-ai/leio-system-prompt') return []
