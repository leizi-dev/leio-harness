...
5. 创建或替换 GitHub 的 `v<version>` 发行版，把差分补丁作为热更新附件上传，并把附件的 HTTPS GitHub 直链写入清单的 `asset.url`。完整 NSIS 安装包只用于首次安装，不能作为热更新附件。
6. 确认发行版直链、文件大小和摘要都不再变化后，再把 `stable.json` 和 `stable.json.sig` 提交到仓库。

公钥已经嵌入桌面端源码。不要提交或上传私钥。不要把约 150MB 的安装包放进 Git；二进制来源使用 GitHub 发行版附件。初始的 `1.0.0` 清单没有安装包资源，因为已有的 `1.0.0` 安装是更新器之前构建的；第一个带更新器的版本需要先人工分发一次，并且使用更高版本号。
