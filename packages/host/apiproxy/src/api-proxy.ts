...
import { installModelSelection } from '@leio-ai/leio-agent'
import type { Agent, ModelSelection, ModelSelectionRef, AgentOptions, AgentStatus } from '@leio-ai/leio-agent'
import type {} from '@leio-ai/leio-agent-presets/types'
import { AttachmentError } from '@leio-ai/leio-attachment'
import type { ImageAttachmentRef } from '@leio-ai/leio-attachment'
import { contentHasImage, createUserMessage, freezeMessage, ReasoningEffortId } from '@leio-ai/leio-llm'
import { errorChain } from '@leio-ai/leio-llm'
import type { ContentBlock, MessageSource } from '@leio-ai/leio-llm'
import { isAppendSurfaceEvent, isJsonValue } from '@leio-ai/leio-session'
import type { JsonValue, Session, SessionEvent, SessionEventMap, SessionHeader, SessionId, UserMessage } from '@leio-ai/leio-session'
import type { SessionPersistence } from '@leio-ai/leio-session-persistence'
import { SessionQueryError, type SessionSearchCursor } from '@leio-ai/leio-session-query'
import { SubagentError } from '@leio-ai/leio-subagent'
import type { SubagentListEntry as CatalogSubagentListEntry } from '@leio-ai/leio-subagent'
import { isUserInvocable } from '@leio-ai/leio-skill'
import type { Workspace, WorkspaceRecord } from '@leio-ai/leio-workspace'
import {
  workspaceDomainState, workspaceRecord, WorkspaceId as brandWorkspaceId,
  WorkspaceMoveInvalidError, WorkspaceOrderInvalidError, WorkspaceUnknownSessionError,
} from '@leio-ai/leio-workspace'
...
} from '@leio-ai/leio-agent-presets'
import type { PresetBearingSession } from '@leio-ai/leio-agent-presets'
import type {} from '@leio-ai/leio-tools'
import type {
...
import type { SessionRawArtifact } from '@leio-ai/leio-session-persistence'
import {
...
import type {} from '@leio-ai/leio-session-projection'
// Type-only: resolves `ctx.get('tasks')` to the background job registry.
import type {} from '@leio-ai/leio-jobs'
import type { JobSnapshot } from '@leio-ai/leio-jobs'
// Type-only: resolves `ctx.get('sessionProjectionCache')` (the cold listing column).
import type {} from '@leio-ai/leio-session-projection-cache'
// GoalError narrows domain rejections to their stable codes at the wire boundary.
import { GoalError } from '@leio-ai/leio-goal'
import type { GoalRef as CoreGoalRef } from '@leio-ai/leio-goal'
// Type-only edges: resolve the command-change stream and `ctx.get('skills')`.
import type {} from '@leio-ai/leio-commands'
...
import type {} from '@leio-ai/leio-cordis-host-runner/types'
import type {} from '@leio-ai/leio-skill'
// The settings/credentials seams: brand guards run at this wire boundary; the
// service reads stay optional (`ctx.get`) so a composition without either
// provider still serves every other domain.
import { SettingsConflictError, settingsNamespace } from '@leio-ai/leio-settings'
import type { SettingsDescriptor, SettingsNamespace, SettingsPathOp } from '@leio-ai/leio-settings'
import { credentialRef } from '@leio-ai/leio-credentials'
// Value edge: the rename impl narrows the title service's validation failure; the import also resolves `ctx.get('sessionTitle')`.
import { SessionTitleInvalidError } from '@leio-ai/leio-session-title'
import type { CallId } from '@leio-ai/leio-llm/brand'
import type { ScopeKey } from '@leio-ai/leio-scope'
import type { ApprovalOutcome, ApprovalRequestId } from '@leio-ai/leio-user-approval'
// Side-effect type import: resolves the `approval/request` waterfall and
// `ctx.get('approval')` without a value dependency on the seam (optional composition).
import type {} from '@leio-ai/leio-user-approval'
...
} from '@leio-ai/leio-user-questions'
import { UserQuestionError } from '@leio-ai/leio-user-questions'
import { DirectoryPickerError } from '@leio-ai/leio-host-directory-picker'
import {
...
function projectionsUnavailableError(): RpcError {
  return {
    code: 'internal',
    message: 'subagent catalog is unavailable: this deployment does not mount the sessionProjections registry (load @leio-ai/leio-session-projection)',
    details: {},
  }
}
...
export function createApiProxy(ctx: Context, defaults: ApiProxyDefaults): ApiProxy {
  ...
  function goalServiceFor(agent: Agent): NonNullable<ReturnType<typeof ctx.get<'goals'>>> | { error: RpcError } {
    ...
    if (goals === undefined) {
      return { error: { code: 'internal', message: 'goal service is absent: neither this session\'s agent preset nor the host composition mounts @leio-ai/leio-goal', details: {} } }
    }
    return goals
  }
  ...
  function settingsAbsent(): RpcError {
    return { code: 'internal', message: 'settings service is absent: this deployment does not mount a settings provider (e.g. @leio-ai/leio-settings-file) in its composition', details: {} }
  }
  ...
  function credentialsAbsent(): RpcError {
    return { code: 'internal', message: 'credentials service is absent: this deployment does not mount a credential provider (e.g. @leio-ai/leio-credentials-local) in its composition', details: {} }
  }
    ...
    sessions: {
      ...
      async search(request, signal) {
        ...
        if (sessionQuery === undefined) {
          return err(request, {
            code: 'internal',
            message: 'session search is unavailable: this deployment does not mount @leio-ai/leio-session-query',
            details: {},
          })
        }
        try {
    ...
    skills: {
      ...
      async list(request) {
        ...
        if (skillRegistry === undefined) {
          return err(request, { code: 'internal', message: 'skill registry is absent: neither this session\'s agent preset nor the host composition mounts @leio-ai/leio-skill', details: {} })
        }
