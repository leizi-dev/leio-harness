...
import { renderPrompt, TOOL_ORDER_REST } from '@leio-ai/leio-system-prompt'
import * as agentCore from '../src/index.ts'
import { agentEvents, type Agent } from '@leio-ai/leio-agent'
...
import type { ToolExecution } from '@leio-ai/leio-tools'
...
/**
 * Unit coverage for the @leio-ai/leio-agent-spine-demo bundle: mounting it brings
...
describe('dsh-agent-spine-demo bundle', () => {
  ...
  it('mounts package companions and forwards invariant selection config', async () => {
    ...
    for (const invariants of [
      { enabled: false },
      { package_allowlist: ['^@leio-ai/leio-agent$'] },
      { package_blocklist: ['^@leio-ai/leio-session$'] },
    ]) {
  ...
  it('loads workspace instructions into requests through the bundled spine', async () => {
    ...
    try {
      ...
      expect(adapter.requests[0]?.system).toContain('You are an AI agent powered by Leio Harness.')
