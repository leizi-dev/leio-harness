...
@leio-ai/leio-llm              abstract LLM service + content-block vocabulary
@leio-ai/leio-session          event-sourced session log + store
@leio-ai/leio-session-title    log-backed title service + deterministic fallback
@leio-ai/leio-system-prompt    prompt-section + tool-schema assembly
@leio-ai/leio-tools            registry + guarded pre/around/post/final-result pipeline
@leio-ai/leio-skill            skill provider registry
@leio-ai/leio-skill-filesystem      local filesystem skill provider
@leio-ai/leio-agent            agent registry + initiator scope + agent/* events
@leio-ai/leio-goal             optional persisted same-session goal domain
@leio-ai/leio-tool-goal        optional model-facing goal controls
@leio-ai/leio-goal-round-driver     optional same-session goal-round driver
@leio-ai/leio-llm-retry        provider-routed request retry policy
@leio-ai/leio-jobs-local      generic background-job registry
@leio-ai/leio-invariants       configurable invariant registry service
@leio-ai/leio-session/invariant
@leio-ai/leio-agent/invariant
@leio-ai/leio-scope/invariant
@leio-ai/leio-agent-loop/invariant
                                  package-owned relational checks
@leio-ai/leio-tool-bash        the model-facing bash schema (unless toolBash=false)
@leio-ai/leio-agent-instructions  AGENTS.md/CLAUDE.md workspace context loader
@leio-ai/leio-tool-skill       session-prefix skill catalog + model-facing loader schema
@leio-ai/leio-tool-jobs       job_output/job_list/job_kill schemas + completion notices
@leio-ai/leio-agent-loop       THE concrete loop (gets the forwarded `agents`)
...
```ts
import type { Config } from '@leio-ai/leio-agent-spine-demo'
...
例如，`{ invariants: { enabled: true, package_allowlist: ['^@leio-ai/leio-'], package_blocklist: ['agent-loop$'] } }` 会让包拥有的配套插件保持挂载，但抑制被阻止的拥有者。Blocklist 匹配优先于 allowlist 匹配；正则表达式与生命周期规则见 [`dsh-invariants`](../../runtime-diagnostics/invariants/README.md)。
