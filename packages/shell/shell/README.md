...
| Package | Role |
|---|---|
| `@leio-ai/leio-shell` (this) | Service Definition: abstract service + vocabulary types |
| `@leio-ai/leio-bash-local` | Service Provider: local subprocesses |
| `@leio-ai/leio-bash-sandbox` | Service Provider: `dsh-bash-local`'s mechanics with every spawn confined via [`ctx.sandbox`](../../sandbox/sandbox/), denials reported as result facts |
| `@leio-ai/leio-tool-bash` | the model-facing tool schemas over `ctx.shell` |
