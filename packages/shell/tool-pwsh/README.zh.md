...
模型看到生成的 [`pwsh` schema](../../../docs/tool-catalog.md#leio-aileio-tool-pwsh)。按 agent 作用域的工具限制可以移除该 agent 的定义。
