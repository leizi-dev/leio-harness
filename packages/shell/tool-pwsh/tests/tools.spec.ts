...
import SystemPrompt, { renderPrompt } from '@leio-ai/leio-system-prompt'
import ToolRuntime, { TOOL_ABORTED, TOOL_ABORTED_BEFORE_DISPATCH } from '@leio-ai/leio-tools'
...
import ApprovalService from '@leio-ai/leio-user-approval'
import type { ApprovalOutcome } from '@leio-ai/leio-user-approval'
import { ShellExecutor } from '@leio-ai/leio-shell'
import type { ShellExecRequest, ShellExecSpec, ShellProcess, ShellRunResult } from '@leio-ai/leio-shell'
...
import type { ShellProcessRead } from '@leio-ai/leio-shell'
...
describe('background execution through the job runtime', () => {
  ...
  it('fails loud when the job runtime is not loaded', async () => {
    ...
    expect(result.isError).toBe(true)
    expect(text(result)).toContain('background jobs unavailable: load @leio-ai/leio-jobs and @leio-ai/leio-tool-jobs')
  })
