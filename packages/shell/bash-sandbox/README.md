...
Sandbox-consuming Service Provider for the [`@leio-ai/leio-shell`](../shell/) executor seam. Load it **instead of** `@leio-ai/leio-bash-local`, together with a [`ctx.sandbox`](../../sandbox/sandbox/) provider (e.g. [`@leio-ai/leio-sandbox-local`](../../sandbox/sandbox-local/)) and a [`ctx.sandboxPolicy`](../../sandbox/sandbox-policy/) (which owns the default mode + workspace root, shared with the sandboxed filesystem) — no alternate tool plugin is needed; `dsh-tool-bash` detects the executor's `sandboxMode` capability and adds the escalation fields.
...
```yaml
- id: sandbox
  name: '@leio-ai/leio-sandbox-local'
- id: sandbox-policy
...
- id: bash
  name: '@leio-ai/leio-bash-sandbox'
```
