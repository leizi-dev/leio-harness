...
async function loadYaml(lines: readonly string[]): Promise<Context> {
  ...
  const modules = new Map<string, unknown>([
    ...
    ['@leio-ai/leio-token-meter', TokenMeter],
    ['@leio-ai/leio-compaction-tool-result-pruner', ToolResultPruner],
    ['@leio-ai/leio-compaction-basic', BasicCompactionEngine],
  ])
...
describe('real Loader composition', () => {
  it('loads the shipped token-meter, pruning, and compaction-basic YAML order', async () => {
    const loaded = await loadYaml([
      ...
      "- name: '@leio-ai/leio-token-meter'",
      "- name: '@leio-ai/leio-compaction-tool-result-pruner'",
      '  config:',
      '    thresholdChars: 100',
      '    headChars: 20',
      '    tailChars: 10',
      "- name: '@leio-ai/leio-compaction-basic'",
      '  config:',
      '    thresholdRatio: 0.5',
      '    retainRatio: 0.125',
      '    auto: false',
