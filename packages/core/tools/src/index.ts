 ...
 * execution pipeline.
 * @module @leio-ai/leio-tools
 */

import { Context, Service } from '@deepseek-ai/cordis'
import z from '@deepseek-ai/schemastery'
import { AnonymousEntries, NamedEntries, ScopedLayers, scopeOf, scopeTarget } from '@leio-ai/leio-scope'
import type { ScopeKey, ScopeLayer, Scoped } from '@leio-ai/leio-scope'
import type { CallId, ContentBlock, ToolSchema } from '@leio-ai/leio-llm'
import { assertNever, deepFreeze, HarnessError } from '@leio-ai/leio-llm'
import type { Agent } from '@leio-ai/leio-agent'
import { snapshotJsonValue } from '@leio-ai/leio-session'
import type { JsonValue, UserMessage } from '@leio-ai/leio-session'
import type { ToolProviderResult } from '@leio-ai/leio-system-prompt'
import type { CodeRuntime } from '@leio-ai/leio-code-runtime'
...
export type { JsonValue } from '@leio-ai/leio-session'
...
// lives in its own UI-facing module; re-export it so `@leio-ai/leio-tools`
...
declare module '@deepseek-ai/cordis' {
  ...
  interface Events {
     ...
     * Scope-filtered dispatch (`@leio-ai/leio-scope`): agent-scoped listeners receive only that agent's calls.
     ...
     * Scope-filtered dispatch (`@leio-ai/leio-scope`): agent-scoped listeners receive only that agent's dispatches.
     ...
     * Scope-filtered dispatch (`@leio-ai/leio-scope`): keyed by `exec.agent`.
...
export interface ToolDefinition extends ToolSchema {
   ...
   * Enforced by `@leio-ai/leio-tool-call-timeout-policy` (a `tools/execute` wrapper); it
 ...
 * @internal
 */
export const TOOL_RUNTIME_SCHEDULER: unique symbol = Symbol('@leio-ai/leio-tools.scheduler')
...
export class ToolRuntime extends Service {
  ...
  private requireCodeRuntime(mode: ToolPresentationMode): CodeRuntime {
    ...
    if (!runtime) {
      throw new Error(`dsh-tools: mode "${mode}" requires a code runtime — load a ctx.codeRuntime implementation (e.g. @leio-ai/leio-code-runtime-worker-thread) or set tools mode to "native"`)
    }
