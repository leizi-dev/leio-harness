...
import type { Scope } from '@leio-ai/leio-scope'
...
import ToolRuntime, { CodeRunFailedError, RUN_CODE_NAME, TOOL_ABORTED_BEFORE_DISPATCH, defineContentToolFixture, defineTool } from '@leio-ai/leio-tools'
import type { Config, JsonSchemaNode, PostToolDecision, ToolExecutionResult } from '@leio-ai/leio-tools'
import type { Agent } from '@leio-ai/leio-agent'
import { Session, SessionId } from '@leio-ai/leio-session'
import type { JsonValue, SessionEventMap } from '@leio-ai/leio-session'
...
describe('per-agent presentation', () => {
  ...
  it('inherits a STANDING preset scope\'s mode down the chain, agents beside it unaffected', async () => {
    const { bindScopeParent } = await import('@leio-ai/leio-scope')
    const { ctx, systemPrompt } = await setup({ mode: 'native' })
    const calls = registerEcho(ctx)
