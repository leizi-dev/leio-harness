...
- **`timeoutMs` on a definition is declarative only** — the registry never enforces deadlines; enforcement requires the `@leio-ai/leio-tool-call-timeout-policy` wrapper.
