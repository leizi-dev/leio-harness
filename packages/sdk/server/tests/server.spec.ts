...
import AgentRegistry, { type Agent, type AgentHandle } from '@leio-ai/leio-agent'
...
import SubagentRuntime, { type SubagentResult, type SubagentRunEndInfo } from '@leio-ai/leio-subagent'
import type { JsonRpcTransportPeer } from '@leio-ai/leio-sdk-protocol'
  ...
  it('rejects unknown JSON-RPC runtime methods', async () => {
    ...
    try {
      ...
      await expect(server.handleRequest('does/not/exist', {}))
        .rejects
        .toThrow('unknown Leio Harness SDK runtime method: does/not/exist')
