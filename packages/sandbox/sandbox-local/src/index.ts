 ...
 * @module @leio-ai/leio-sandbox-local
 */
...
import { assertNever } from '@leio-ai/leio-llm'
import { SandboxProvider, SandboxUnavailableError } from '@leio-ai/leio-sandbox'
import type { ConfinedArgv, ConfinedSandboxMode, RunnerFailureRule, SandboxEnforcement, SandboxPolicy } from '@leio-ai/leio-sandbox'
import type { SessionId } from '@leio-ai/leio-session'
import { AclWriteGrant, assertTempRootOutsideWorkspace, tempWriteSid, workspaceWriteSid } from '@leio-ai/leio-sandbox-windows-acl'
...
const PLATFORM_CHAINS: Record<string, readonly SelectedRunner['runner'][]> = {
  ...
  darwin: ['seatbelt'],
  // The Windows restricted-token runner (@leio-ai/leio-sandbox-windows-acl):
...
export class LocalSandboxProvider extends SandboxProvider {
  ...
  private windowsAclRunnerInvocation(): string[] {
    ...
    const builtEntry = this.internals.windowsAclRunnerEntry ?? fileURLToPath(import.meta.resolve('@leio-ai/leio-sandbox-windows-acl/runner'))
    if (existsSync(builtEntry)) return [process.execPath, builtEntry]
    const sourceEntry = fileURLToPath(import.meta.resolve('@leio-ai/leio-sandbox-windows-acl/src/runner.ts'))
