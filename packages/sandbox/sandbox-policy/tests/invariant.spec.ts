...
import SessionStore, { type Session, type SessionEvent } from '@leio-ai/leio-session'
import InvariantRegistry, { InvariantError } from '@leio-ai/leio-invariants'
import * as SandboxPolicyInvariant from '@leio-ai/leio-sandbox-policy/invariant'
...
describe('sandbox-policy invariants', () => {
  ...
  it('rejects and attributes an unknown durable sandbox mode', async () => {
    ...
    expect(() => { ctx.emit('session/event', {} as Session, modeEvent('host-root')) })
      .toThrow(new InvariantError('@leio-ai/leio-sandbox-policy', 'sandbox/mode carries unknown mode "host-root"'))
  })
  ...
  it('rejects an unknown mode already present on late registration', async () => {
    ...
    await expect(ctx.plugin(SandboxPolicyInvariant).then(() => undefined)).rejects.toMatchObject({
      code: 'INVARIANT',
      packageName: '@leio-ai/leio-sandbox-policy',
    })
  })
})
