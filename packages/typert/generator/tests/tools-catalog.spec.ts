...
import type { TypertContribution } from '@leio-ai/leio-typert-registry/types'
import { EVENT_API, SERVICE_API, TYPE_API } from '@leio-ai/leio-tool-cordis/src/api-catalog.ts'
...
describe('model-driven dsh-tools generation', () => {
  it('round-trips the complete service and event structure through the runtime registry', { timeout: 30_000 }, async () => {
    ...
    const artifact = new FaceModelEmitter(host).emit('@leio-ai/leio-tools')
    ...
    const record = ctx.typert.getPackage('@leio-ai/leio-tools', 'host')
    ...
    await dispose()
    expect(ctx.typert.getPackage('@leio-ai/leio-tools', 'host')).toBeUndefined()
  })
})
