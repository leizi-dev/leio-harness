...
} from '@leio-ai/leio-typert-registry'
import type {
  InvocationDescriptor,
  TypertContext,
  TypertLookup,
  TypertRemoteContribution,
} from '@leio-ai/leio-typert-protocol'
...
describe('TypertRegistry', () => {
  it('registers and queries generated schemas separately from package reflection', async () => {
    ...
    expect(typertKey('@leio-ai/leio-tools', 'ToolInput')).toBe('@leio-ai/leio-tools#ToolInput')
    expect(typertPackageKey('@leio-ai/leio-tools', 'host')).toBe('@leio-ai/leio-tools#host')
    expect(ctx.typert.get('@leio-ai/leio-tools#ToolInput')).toMatchObject({
      package: '@leio-ai/leio-tools',
      face: 'host',
      name: 'ToolInput',
    })
    expect(ctx.typert.get('@leio-ai/leio-tools#ToolInput')?.schema).toBe(contribution.schemas[0]?.schema)
    expect(ctx.typert.getPackage('@leio-ai/leio-tools', 'host')).toMatchObject({
      key: '@leio-ai/leio-tools#host',
  ...
  it('withdraws schemas and package metadata through the exact contribution disposer', async () => {
    ...
    expect(ctx.typert.getPackage('@leio-ai/leio-tools')).toBeDefined()

    await dispose()

    expect(ctx.typert.get('@leio-ai/leio-tools#ToolInput')).toBeUndefined()
    expect(ctx.typert.getPackage('@leio-ai/leio-tools')).toBeUndefined()
  ...
  it('rejects duplicate package faces and schema keys before committing', async () => {
    ...
    expect(ctx.typert.get('@leio-ai/leio-tools#ToolInput')?.schema).toBe(original.schemas[0]?.schema)
  ...
  it('resolves required schemas and projects fresh JSON Schema documents', async () => {
    ...
    expect(ctx.typert.resolve('@leio-ai/leio-tools#ToolInput').name).toBe('ToolInput')
    expect(() => ctx.typert.resolve('@leio-ai/leio-tools#Missing')).toThrow('contributes no schema named "Missing"')
    ...
    const projected = ctx.typert.toJSONSchema('@leio-ai/leio-tools#ToolInput')
    expect(projected).toMatchObject({ type: 'object', properties: { name: { type: 'string' } } })
    expect(ctx.typert.toJSONSchema('@leio-ai/leio-tools#ToolInput')).not.toBe(projected)
  })
