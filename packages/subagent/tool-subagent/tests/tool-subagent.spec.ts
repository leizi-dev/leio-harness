...
import ToolRuntime, { TOOL_ABORTED_BEFORE_DISPATCH } from '@leio-ai/leio-tools'
import { assembleContextFor, type Agent } from '@leio-ai/leio-agent'
...
import type { SubagentStartRequest } from '@leio-ai/leio-subagent'
...
describe('dsh-tool-subagent background mode', () => {
  ...
  it('fails loud when the tasks runtime is not loaded', async () => {
    ...
    expect(result.isError).toBe(true)
    expect(text(result)).toContain('background jobs unavailable: load @leio-ai/leio-jobs')
  })
