...
import LlmRuntime, { createUserMessage, LlmAdapter  } from '@leio-ai/leio-llm'
import type { GenerateOptions, StreamChunk } from '@leio-ai/leio-llm'
import SessionStore, { SessionId } from '@leio-ai/leio-session'
import SessionTitleService from '@leio-ai/leio-session-title'
import * as providerPlugin from '@leio-ai/leio-session-title-first-prompt-llm'
...
async function loadComposition(): Promise<Context> {
  ...
  await writeFile(configPath, [
    "- name: '@leio-ai/leio-llm'",
    "- name: '@leio-ai/leio-session'",
    "- name: '@leio-ai/leio-session-title'",
    '  config:',
    '    fallbackMaxWords: 5',
    '    fallbackMaxBytes: 40',
    '    maxTitleBytes: 80',
    "- name: '@leio-ai/leio-session-title-first-prompt-llm'",
    '  config:',
    '    targetWords: 5',
  ...
  const modules = new Map<string, unknown>([
    ['@leio-ai/leio-llm', LlmRuntime],
    ['@leio-ai/leio-session', SessionStore],
    ['@leio-ai/leio-session-title', SessionTitleService],
    ['@leio-ai/leio-session-title-first-prompt-llm', providerPlugin],
  ])
