...
| Package | Role |
|---|---|
| `@leio-ai/leio-web` (this) | Service Definition: the service, provider registries, selection policy, request/result vocabulary, the `WebError` taxonomy |
| `@leio-ai/leio-web-search-exa` | Search provider: Exa |
| `@leio-ai/leio-web-search-perplexity` | Search provider: Perplexity |
| `@leio-ai/leio-web-fetch-http` | Fetch provider: anonymous public HTTP(S) |
| `@leio-ai/leio-tool-web` | Consumer: the model-facing `web_search` / `web_fetch` tool schemas over `ctx.web` |
