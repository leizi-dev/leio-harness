...
/**
 * Keyless built-artifact smoke: plain Node imports `@leio-ai/leio-lsp` and
 * `@leio-ai/leio-lsp-stdio` by name through their exports maps, spawns the fixture server, runs
...
describe.skipIf(!built)('built lib real load path (plain node)', () => {
  it('runs a query through lib/index.js and disposes cleanly, framing over the base protocol', async () => {
    ...
    const script = `
      ...
      const { default: Lsp } = await import('@leio-ai/leio-lsp')
      const LspLocal = await import('@leio-ai/leio-lsp-stdio')
      const { default: LocalFileSystem } = await import('@leio-ai/leio-fs-local')
      const { default: LocalSubprocessRuntime } = await import('@leio-ai/leio-subprocess-local')
