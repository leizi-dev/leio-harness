...
import LocalCredentialProvider from '@leio-ai/leio-credentials-local'
import { settingsNamespace } from '@leio-ai/leio-settings'
import FileSettingsProvider from '@leio-ai/leio-settings-file'
import { getOrCreateAnonymousUserId } from '@leio-ai/leio-anonymous-user-id'
...
async function loadComposition(
  ...
  await writeFile(configPath, [
    ...
    ...options.withDynamic
      ? [
        '- id: settings',
        "  name: '@leio-ai/leio-settings-file'",
        '  config:',
        `    path: ${JSON.stringify(settingsPath)}`,
        '    debounceMs: 10',
        '- id: credentials',
        "  name: '@leio-ai/leio-credentials-local'",
        '  config:',
  ...
  const modules = new Map<string, unknown>([
    ...
    ['@leio-ai/leio-settings-file', FileSettingsProvider],
    ['@leio-ai/leio-credentials-local', LocalCredentialProvider],
    ['@leio-ai/leio-llm-deepseek', LlmDeepSeek],
  ])
  ctx.loader.internal = {
    version: 'v2',
