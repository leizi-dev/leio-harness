...
import * as StorageDomain from '@leio-ai/leio-storage-domain'
import * as StorageJson from '@leio-ai/leio-storage-json'
import { remoteMethods } from '@leio-ai/leio-typert-protocol'
...
async function loadComposition(configPath: string): Promise<Context> {
  ...
  const modules = new Map<string, unknown>([
    ['@leio-ai/leio-session', SessionStore],
    ['@leio-ai/leio-session-persistence-jsonl', JsonlSessionPersistence],
    ['@leio-ai/leio-storage', Storage],
    ['@leio-ai/leio-storage-json', StorageJson],
    ['@leio-ai/leio-storage-domain', StorageDomain],
    ['@leio-ai/leio-message-feedback', MessageFeedbackService],
  ])
  ctx.loader.internal = {
    version: 'v2',
...
describe('message feedback through a real Loader composition', () => {
  it('persists a checkpointed target and its sidecar across a cold restart', async () => {
    ...
    await writeFile(configPath, [
      "- name: '@leio-ai/leio-session'",
      "- name: '@leio-ai/leio-session-persistence-jsonl'",
      '  config:',
      `    root: ${JSON.stringify(join(root, 'sessions'))}`,
      '    compression: none',
      '    writeBatchMaxDelayMs: 1',
      "- name: '@leio-ai/leio-storage'",
      "- name: '@leio-ai/leio-storage-json'",
      '  config:',
      `    root: ${JSON.stringify(join(root, 'storage'))}`,
      "- name: '@leio-ai/leio-storage-domain'",
      '  config:',
      '    backend: json',
      "- name: '@leio-ai/leio-message-feedback'",
      '  config:',
      '    maxNoteBytes: 32',
      '',
    ].join('\n'))
