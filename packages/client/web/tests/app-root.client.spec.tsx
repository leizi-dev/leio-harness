...
afterEach(cleanup)
import { AppRoot } from '@leio-ai/leio-client-web/src/AppRoot.tsx'
import { createLoaderStatusStore, createSignal } from '@leio-ai/leio-client-web/src/loader-status.ts'
...
describe('AppRoot', () => {
  ...
  it('lists failed entries and stays on the loading page', () => {
    ...
    act(() => {
      status.set('@leio-ai/leio-client-ui-layout', 'failed')
      status.set('ok', 'active')
    })
    expect(getByText('Failed to load plugins')).toBeTruthy()
    expect(getByText('@leio-ai/leio-client-ui-layout')).toBeTruthy()
    expect(queryByTestId('real-ui')).toBeNull()
  })
