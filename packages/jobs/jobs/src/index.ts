 ...
 * process-local registry lives in `@leio-ai/leio-jobs-local`.
 * @module @leio-ai/leio-jobs
 */
...
export abstract class JobRegistry extends Service {
  constructor(ctx: Context) {
    ...
    if (new.target === JobRegistry) {
      throw new Error('@leio-ai/leio-jobs is the abstract job registry seam; load an implementation such as @leio-ai/leio-jobs-local instead')
    }
    super(ctx, 'jobs')
  }
