 ...
 * @module @leio-ai/leio-jobs-local
 */

import { Context } from '@deepseek-ai/cordis'
import z from '@deepseek-ai/schemastery'
import type { Agent } from '@leio-ai/leio-agent'
import { AnonymousEntries, ScopedLayers, scopeOf } from '@leio-ai/leio-scope'
import type { ScopeLayer } from '@leio-ai/leio-scope'
import { deadline, timeoutOf } from '@leio-ai/leio-timeout'
import { JobRegistry, JobId } from '@leio-ai/leio-jobs'
import type {
  JobDoneListener, JobKind, JobOutcome, JobRead, JobSnapshot, JobStart, JobStatus,
  JobsChangedListener,
} from '@leio-ai/leio-jobs'
 ...
 * `@leio-ai/leio-jobs` for the ownership, isolation, and lifecycle
...
export class LocalJobRegistry extends JobRegistry {
  ...
  start(spec: JobStart): JobId {
    if (!this.servesOwner(spec.owner)) {
      throw new Error('background jobs unavailable: no job controller serves this agent (load @leio-ai/leio-tool-jobs in its composition)')
    }
  ...
  private ensureOwnerCleanup(owner: Agent): void {
    ...
    if (agents === undefined) {
      throw new Error('background job ownership requires the agent registry (load @leio-ai/leio-agent)')
    }
