...
工作区包使用 `@leio-ai/leio-*` 作用域，根项目名称为 `leio-harness`。Web 和桌面应用显示 `Leio Harness`，并将用户提供的 Leio PNG 用于所有应用图片：侧边栏品牌图标、浏览器 favicon、PWA manifest、Electron 窗口、Windows 可执行文件、安装程序和快捷方式。DeepSeek provider 名称、模型标识符、API endpoint 以及 `DEEPSEEK_*` 凭据设置仍属于 provider，不因本次产品品牌修改而重命名。
...
- Web 应用加载 `apps/web/public/leio-icon.png`；桌面 builder 按其 package root 要求加载字节完全相同的 `apps/desktop/build/icon.png`。
