...
1. `@leio-ai/leio-web` (`packages/web/web`) owns `ctx.web`, provider registration, provider selection, shared request/result vocabulary, and web-specific errors.
2. Provider packages implement concrete backends and register capabilities with `ctx.web`, for example `@leio-ai/leio-web-search-exa`, `@leio-ai/leio-web-search-perplexity`, `@leio-ai/leio-web-search-deepseek`, and `@leio-ai/leio-web-fetch-http`.
3. `@leio-ai/leio-tool-web` (`packages/web/tool-web`) owns the model-facing `web_search` and `web_fetch` tool schemas, prompt sections, argument validation, result formatting, and tool-owned presentation over `ctx.web`.
...
```text
@leio-ai/leio-tool-web  --depends on-->  @leio-ai/leio-web  <--depends on--  @leio-ai/leio-web-search-exa
        consumer                                 interface                       implementation
                                                                 <--depends on--  @leio-ai/leio-web-search-perplexity
                                                                                  implementation
                                                                 <--depends on--  @leio-ai/leio-web-search-deepseek
                                                                                  implementation
                                                                 <--depends on--  @leio-ai/leio-web-fetch-http
                                                                                  implementation
```
...
```mermaid
flowchart LR
  exa["@leio-ai/leio-web-search-exa"] -->|registerSearchProvider| web["@leio-ai/leio-web / ctx.web"]
  perplexity["@leio-ai/leio-web-search-perplexity"] -->|registerSearchProvider| web
  deepseek["@leio-ai/leio-web-search-deepseek"] -->|registerSearchProvider| web
  fetchLocal["@leio-ai/leio-web-fetch-http"] -->|registerFetchProvider| web
  toolWeb["@leio-ai/leio-tool-web"] -->|search/fetch| web
  toolWeb -->|ctx.tools.register| webSearch["tool: web_search"]
  toolWeb -->|ctx.tools.register| webFetch["tool: web_fetch"]
```

`@leio-ai/leio-web` depends only on Cordis and low-level harness support. It declares `ctx.web`, provider interfaces, request/result types, the provider availability contract, and error codes. It does not import tool, agent, session, LLM, or provider packages.
...
`@leio-ai/leio-tool-web` depends on `@leio-ai/leio-web`, `@leio-ai/leio-tools`, `@leio-ai/leio-system-prompt`, and Cordis. It never imports concrete provider packages.
...
```ts
import type { WebFetchRequest, WebFetchResult, WebSearchRequest, WebSearchResult } from '@leio-ai/leio-web'
...
```yaml
- id: web
  name: '@leio-ai/leio-web'
  config:
    searchProvider: exa
    fetchProvider: http

- id: web-search-exa
  name: '@leio-ai/leio-web-search-exa'

- id: web-search-perplexity
  name: '@leio-ai/leio-web-search-perplexity'

- id: web-search-deepseek
  name: '@leio-ai/leio-web-search-deepseek'

- id: web-fetch-http
  name: '@leio-ai/leio-web-fetch-http'
