...
| Package | Role |
|---|---|
| `@leio-ai/leio-spill` | Interface: `ctx.spillStore`, vocabulary types, no storage implementation. |
| `@leio-ai/leio-spill-local` | Local backend: private, session-scoped file storage on the host filesystem. |
| `@leio-ai/leio-spill-policy` | Tool-result policy plugin: wraps final text results after dispatch and replaces oversized results with a retained preview plus a spill locator. |
...
- id: spill-local
  name: '@leio-ai/leio-spill-local'

- id: spill-policy
  name: '@leio-ai/leio-spill-policy'
  config:
...
- `@leio-ai/leio-output-retention` owns preview mechanics (`TextRetainer`, `ItemRetainer`, and omitted metadata).
- `@leio-ai/leio-spill` owns saving final text and returning a locator plus retrieval hint.
- `@leio-ai/leio-spill-policy` applies the default final-result policy in the tool pipeline, composing the two.
