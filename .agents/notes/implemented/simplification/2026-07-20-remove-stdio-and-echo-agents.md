...
- `@leio-ai/leio-tui` owns terminal-interactive execution. It rejects non-TTY streams before Loader boot; `apps/cli/config/base.cordis.yml` plus the `tui.cordis.yml` overlay own the complete coding composition, with PTY plus terminal-snapshot coverage in `apps/cli/tests/`.
...
- [`@leio-ai/leio-acp-demo`](../../../../packages/examples/acp-demo/README.md) and `@leio-ai/leio-sdk-jsonrpc-server` own their framed protocol integrations.
