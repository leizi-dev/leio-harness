...
`SubagentRuntime` is the only public service. It exposes ordinary `start(name, request)`, Task-backed `startContinuable(spec)`, and intent-named `followup(...)`; provider resume dispatch remains private to its continuation manager. The standalone `@leio-ai/leio-subagent-control` package and `ctx.subagentControl` key are absent; the optional `@leio-ai/leio-tool-subagent-control` package injects `ctx.subagents` directly.
