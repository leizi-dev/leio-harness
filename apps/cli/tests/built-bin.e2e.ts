...
import { startMockLlmServer } from '@leio-ai/leio-llm-mock-server'
...
function createEnvironmentProbeProfile(home: string, project: string): void {
  ...
  writeFileSync(join(profileDir, 'package.json'), JSON.stringify({
    ...
    private: true,
    dependencies: {},
    dsh: { profile: { bundles: ['@leio-ai/leio-base'] } },
  }, undefined, 2))
  writeFileSync(join(profileDir, 'cordis.patch.yml'), [
    '- insert:',
 ...
 * `@leio-ai/leio-cmdline` and `commander` through the profile module
...
function createStartupFixture(): StartupFixture {
  ...
  writeFileSync(join(bundleDir, 'startup.mjs'), [
    ...
    "import { parseCmdline } from '@leio-ai/leio-cmdline'",
...
describe.skipIf(!existsSync(dshBin))('dsh BUILT bin (node lib/bin.js, no tsx)', () => {
  ...
  it('activates a dependency that gained dsh.bundle in a later update', async () => {
    ...
    try {
      ...
      expect(manifest.dsh.profile.bundles).toEqual(['@leio-ai/leio-base'])
      ...
      expect(manifest.dsh.profile.bundles).toEqual(['@leio-ai/leio-base', 'late-bundle'])
    } finally {
      rmSync(home, { recursive: true, force: true })
    }
  }, 30_000)
  ...
  describe('config dump', () => {
    ...
    it('prints the web profile bundle layers without a user layer', async () => {
      ...
      expect(code).toBe(0)
      expect(stderr).toBe('')
      expect(stdout).toContain("name: '@leio-ai/leio-agent-loop'")
      expect(stdout).toContain('agents: []')
      expect(stdout).toContain('# == @leio-ai/leio-base')
      expect(stdout).toContain("name: '@leio-ai/leio-host-webserver'")
    }, 30_000)
    ...
    it('prints the headless profile without Host or browser layers', async () => {
      ...
      expect(code).toBe(0)
      expect(stderr).toBe('')
      expect(stdout).toContain("name: '@leio-ai/leio-headless'")
      expect(stdout).not.toMatch(/name: '@leio-ai\/leio-host-/)
      expect(stdout).not.toContain("name: '@leio-ai/leio-web-app'")
      expect(stdout).not.toMatch(/name: '@leio-ai\/leio-client-/)
    }, 30_000)
