...
import { composeEntries, initProfile, loadProfile, PROFILES_DIR } from '@leio-ai/leio-app-boot'
...
describe('the shipped shell composition (real bundle layers)', () => {
  ...
  it('composes the confined pwsh roster on win32 and the bash roster on POSIX from the same rows', () => {
    ...
    initProfile(join(home, PROFILES_DIR, 'web'), ['@leio-ai/leio-base', '@leio-ai/leio-web-app'])
    ...
    for (const name of ['@leio-ai/leio-pwsh-sandbox', '@leio-ai/leio-tool-pwsh']) {
  ...
  it('base-only profiles carry both stacks with the same platform gating', () => {
    home = mkdtempSync(join(tmpdir(), 'dsh-windows-home-'))
    initProfile(join(home, PROFILES_DIR, 'base-only'), ['@leio-ai/leio-base'])
