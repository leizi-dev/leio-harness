...
import { scrubRequestHeaders, stabilizeFixtureMessageIds } from '@leio-ai/leio-acp-snapshot'
import {
  assertEntriesLoaded,
  composeEntries,
  healProfilesModuleFallback,
  loadOverlayPatches,
} from '@leio-ai/leio-app-boot'
import { dshHomePath } from '@leio-ai/leio-home-paths'
import { settingsNamespace } from '@leio-ai/leio-settings'
import { LlmAdapter } from '@leio-ai/leio-llm'
import type {
  LlmModelInfo, LlmProviderInfo, LlmResolvedModelInfo, StreamChunk,
} from '@leio-ai/leio-llm'
import type { ReplayHandle } from '@leio-ai/leio-llm-replay'
import { installLlmReplay, parseSessionLog } from '@leio-ai/leio-llm-replay'
import SessionStore, {
  packChunkRuns,
  ...
  type SessionEvent,
  type SessionHeader,
} from '@leio-ai/leio-session'
import JsonlSessionPersistence from '@leio-ai/leio-session-persistence-jsonl'
// Empty type imports carry the webServer/agents/sessionPersistence Context merges.
import type {} from '@leio-ai/leio-host-webserver'
import type {} from '@leio-ai/leio-agent'
import { provideCmdline } from '@leio-ai/leio-cmdline'
...
// } from '@leio-ai/leio-client-ui-settings-models'
...
export const WELCOME_NOTICE_COPY = {
  zh: {
    title: '内测声明',
    body: 'Leio Harness 目前的 0.1 版本仍处在面向 Harness 开发者进行测试的阶段，还有许多地方需要持续改进和打磨，希望听取广大开发者的反馈建议。预计 Leio Harness 的核心插件以及基础 API 都会在接下来的一段时间内快速迭代、持续演化。\n\n我们期待与全球开发者一起，在开源、开放、可复用、可组合的基础设施之上，共同探索智能上限。欢迎全球 Harness 开发者加入 DSH 插件生态。',
    continueLabel: '继续',
  },
} as const
...
export async function launchWebScaffold(options: LaunchOptions = {}): Promise<WebScaffold> {
  ...
  const patches: PatchOptions[] = [
    ...
    // (apps/web IS @leio-ai/leio-web-frontend); only the URL line is silenced.
    ...
    { insert: [
      { id: 'directory-picker-browse', name: '@leio-ai/leio-host-directory-picker-browse' },
      { id: 'ui-directory-picker-browse', name: '@leio-ai/leio-client-ui-directory-picker-browse' },
    ] },
    ...
    ...options.cordisTools === true
      ? [{ insert: [
        { id: 'tool-cordis', name: '@leio-ai/leio-tool-cordis' },
      ] }]
      : [],
