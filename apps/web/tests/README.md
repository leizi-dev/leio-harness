...
## Do not import `@leio-ai/leio-client-*` here
...
`@leio-ai/leio-goal/remote`, so the Host build phase ended up waiting on an
...
it imports `AppWebEntry` from `@leio-ai/leio-client-web` and the boot-manifest
type from `@leio-ai/leio-client-modules/client`: booting the real shell is what
...
`@leio-ai/leio-client-runtime/client` because `client/runtime` is reachable
