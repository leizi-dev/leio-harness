...
const UI_PLUGIN_DIRS = [
  ...
  'ui-settings', 'ui-settings-general', 'ui-settings-update', 'ui-settings-models', 'ui-conversation',
...
describe.skipIf(!process.env.DEEPSEEK_API_KEY || notReady.length > 0)('web smoke (real host, real key)', () => {
  ...
  it('empty-state first send completes a real model round', async () => {
    ...
    await page.waitForFunction(
      () => document.title !== 'Leio Harness' && document.title.endsWith(' — Leio Harness'),
      undefined,
      { timeout: 15_000 },
    )
    ...
    await page.waitForFunction(
      expected => document.title === `${expected} — Leio Harness`,
      durableTitle,
      { timeout: 15_000 },
    )
